...
const partitionTemplates = (templates, directory) => {
  ...
  for (const template of templates) {
    const parsed = filesUtil.readFile(`${prefix}${template.template}`);
    const tree = templatesUtil.parse(parsed);
  ...
  for (const file of files) {
    ...
    for (const template of templates) {
      ...
      if (template && template.tree) {
        contents = templatesUtil.reconstruct(template.tree, metadata, html);
      }
...
const transformListings = (templates, posts, { dateFormat, pages }) => {
  ...
  for (const template of templates) {
    ...
    for (const output of transformed) {
      ...
      listings.push({
        contents: templatesUtil.reconstruct(tree, {
          ...output.metadata,
          files: output.files,
        }),
...
const getTransformedTemplate = (template, directory) => {
  ...
  return templatesUtil.parse(parsed);
};
